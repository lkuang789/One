...
raw_yuv_path = '/data/lk/datasets/MFQEv2_dataset/test_18/raw/Traffic_2560x1600_150.yuv'
lq_yuv_path = '/data/lk/datasets/MFQEv2_dataset/test_18/HM16.5_LDP/QP22/Traffic_2560x1600_150.yuv'
h, w, nfs = 1600, 2560, 100  # 视频分辨率和帧数

# 输出目录
output_dir = "output_frames/QP22/Traffic_2560x1600_150"
...
def main():
    ...
    pbar = tqdm(total=min(nfs, 50), ncols=80)
    ...
    for idx in range(min(nfs, 50)):
        ...
        for idx_ in idx_list:
            input_data.append(torch.from_numpy(lq_y[idx_]).cuda())
        input_data = torch.stack(input_data)  # 批量堆叠为一个 Tensor
        input_data = input_data.unsqueeze(0)  # 扩展维度
        ...
        # 释放显存
        del input_data, enhanced_frm, gt_frm
