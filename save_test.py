...
def process_video(model, raw_path, lq_path, h, w, nfs, output_dir):
    ...
    for idx in range(min(nfs, 50)):
        ...
        # 同时保存三种帧
        save_frame(enhanced[0, 0].cpu().numpy(), lq_u[idx], lq_v[idx],  # 增强帧
                   os.path.join(enh_dir, f"frame_{idx:04d}.png"))
        save_frame(lq_y[idx], lq_u[idx], lq_v[idx],  # 低质量帧
                   os.path.join(lq_dir, f"frame_{idx:04d}.png"))
        save_frame(raw_y[idx], raw_u[idx], raw_v[idx],  # 原始帧
                   os.path.join(raw_dir, f"frame_{idx:04d}.png"))
